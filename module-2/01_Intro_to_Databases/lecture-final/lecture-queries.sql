...
SELECT state_abbreviation, city_name FROM city WHERE state_abbreviation <> 'CA'; -- Or use != for not equals
...
SELECT city_name, state_abbreviation, area, population FROM city WHERE area < 25 AND population > 100000;
...
SELECT city_name, area, population FROM city WHERE state_abbreviation IN ('CA', 'FL'); -- state_abbreviation = 'CA' OR state_abbreviation = 'FL';
...
SELECT park_name, area, area * 1000 * 1000 AS area_in_square_meters FROM park;
...
SELECT city_name FROM city ORDER BY city_name; -- Contains some duplicates

-- Do it again, but without the repeats (note the difference in row count)
SELECT DISTINCT city_name FROM city ORDER BY city_name; -- DISTINCT removes duplicates
...
SELECT city_name FROM city WHERE city_name LIKE 'C_n%';
...
SELECT park_name, area FROM park WHERE area BETWEEN 100 AND 200; -- Same as stmt above
