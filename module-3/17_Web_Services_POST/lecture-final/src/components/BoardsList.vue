  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      boardsService
        ...
        .catch((err) => {
          if (err.response) {
            this.errorMsg = "Error creating board: " + err.response.statusText;
          } else if (err.request) {
            this.errorMsg = "Error creating board: server not found";
          } else {
            this.errorMsg =
              "error creating board. could not create the request";
          }
        });
    },
